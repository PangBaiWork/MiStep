...
public class Main extends BaseActivity implements View.OnClickListener, CompoundButton.OnCheckedChangeListener, View.OnLongClickListener {
    ...
    void initControlAttribute() {
        ...
        EditText text=findViewById(R.id.edtAddRandomSteps);
        SwitchMaterial random=findViewById(R.id.shRandomStep);
        String mRandom= Shared.getShared().getValue(Main.USER_SETTING,"random", "");
       if (!mRandom.isEmpty()){
           random.setChecked(true);
           text.setText(mRandom);
       }
        random.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {

                String range=  text.getText().toString();
                if (b&&range.isEmpty()){
                    random.setChecked(false);
                    Toast.makeText(Main. this,"not empty",Toast.LENGTH_SHORT).show();
                }
                shared.putValue(USER_SETTING, "random",b? range:"").commitShared(USER_SETTING);
